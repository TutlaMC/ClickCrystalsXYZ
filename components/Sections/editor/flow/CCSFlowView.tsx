...
function getPortDataType(
  ...
  return (
    [...data.inputs, ...data.outputs].find((p) => p.id === handleId)
      ?.dataType ?? null
  );
}
...
function CCSFlowViewInner({ code, onCodeChange }: Props) {
          ...
          _onChange: (patch: Partial<CommandNodeData>) => {
            setNodes((prev) => {
              const next = prev.map((p) =>
                p.id === n.id ? { ...p, data: { ...p.data, ...patch } } : p,
              );
  ...
  const isValidConnection = useCallback(
    (conn: Connection): boolean => {
      ...
      const srcType = getPortDataType(
        conn.source!,
        conn.sourceHandle,
        getNodes(),
      );
      const tgtType = getPortDataType(
        conn.target!,
        conn.targetHandle,
        getNodes(),
      );
      ...
      const newNode: Node<CommandNodeData> = {
        ...
        data: {
          ...
          _onChange: (patch: Partial<CommandNodeData>) => {
            setNodes((prev) => {
              const next = prev.map((p) =>
                p.id === nodeId ? { ...p, data: { ...p.data, ...patch } } : p,
              );
              const newCode = generateCCS(next, getEdges());
              if (newCode !== lastEmitted.current) {
                lastEmitted.current = newCode;
  ...
  const wrappedNodesChange: typeof onNodesChange = useCallback(
    (changes) => {
  ...
  const wrappedEdgesChange: typeof onEdgesChange = useCallback(
    (changes) => {
      ...
      >
        {nodes.length === 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 10 }}>
              Write `def module my-module` or drag a Module node
            </span>
          </div>
        )}
        <ReactFlow
        ...
        >
          ...
          <MiniMap
            ...
            nodeColor={(n) =>
              CATEGORY_COLORS[(n.data as CommandNodeData)?.category] ?? '#333'
            }
          />
        </ReactFlow>
      </div>
